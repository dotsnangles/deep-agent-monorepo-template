...
from src.core.config import DATABASE_URL, LLM_PROVIDER, REDIS_URL
...
async def health(request: Request):
    ...
    return {
        ...
        "checkpointer": (
            "AsyncPostgresSaver" if getattr(app.state, "checkpointer", None) else "MemorySaver"
        ),
        "store": "AsyncPostgresStore" if getattr(app.state, "store", None) else "None",
    }
