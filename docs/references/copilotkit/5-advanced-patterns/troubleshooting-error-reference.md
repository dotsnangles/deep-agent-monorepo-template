...
**Where:** the **v2** provider, ``, imported from
...
> [!NOTE]
> The **v1** `` provider throws in **both development and
>   production** when nothing is configured, whereas the v2 ``
...
// Agents you manage yourself — the `selfManagedAgents` prop is also accepted by the v1 `` wrapper
{children}
```
