...
| Chat Engine | In-process state machine encapsulating linear message sequence mutations, stream chunk accumulation, and optimistic rollback | Web Client (`apps/web`) |
...
| Chat Repository | Deep domain repository interface encapsulating session/message CRUD, linear queries, and transactional forking mutations | Database (`@repo/db`) |
| Drizzle Chat Repository | Concrete Drizzle ORM implementation providing atomic database transactions for session forking and message insertion | Database (`@repo/db`) |
