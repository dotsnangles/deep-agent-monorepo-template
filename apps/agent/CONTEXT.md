...
| Deep Agent Harness | Unified LangChain `create_deep_agent` instance orchestrating declarative HITL (`interrupt_on`), context offloading, skills, and memory | `src/domain/factory.py` |
| Domain Ports | Strict abstract protocol seams decoupling AI domain logic from infrastructure dependencies (`PersistencePort`, `SandboxExecutionPort`, `StoragePort`, `ModelProviderPort`) | `src/domain/ports.py` |
...
| Graph Registry | Registry managing compilable graph factories (Chat, Data Analysis, Code) and routing by `agent_type` | `src/domain/registry.py` |
...
| Todo Planning Middleware | Harness middleware (`TodoListMiddleware`) enabling structured task decomposition and status tracking via `write_todos` | `src/domain/middleware.py` |
| Subagent Delegation Channel | Ephemeral child agent execution layer spawned via `task` tool with isolated context windows and final report compression | `src/domain/subagents.py` |
...
| Thread ID | Unique session identifier mapping 1:1 with LangGraph state checkpoints and Redis Pub/Sub channels | `src/runtime/types.py` |
...
| Declarative Tool Approval | Tool gating mechanism using `interrupt_on` mapping tool names to human-in-the-loop review before execution | `src/domain/factory.py` |
...
| Environment-Aware Agent Factory | Factory dynamically assembling Deep Agent graphs with tailored profiles, backends, permissions, and middleware per active environment | `src/domain/factory.py` |
| General-Purpose Subagent | Native super-model inheriting subagent topology for on-demand context quarantine without pre-declared personas | `src/domain/subagents.py` |
| Rubric Quality Loop | Self-correction iterative evaluation middleware (`RubricMiddleware`) using LLM-as-a-judge for automated quality gates | `src/domain/factory.py` |
| Fault Tolerance Package | Layered resilience suite (`ModelFallback`, `ToolRetry`, `ModelCallLimit`, `ToolCallLimit`) preventing runaway loops and outages | `src/domain/factory.py` |
| Context Compaction Engine | Dual-action context management combining automatic 85% window summarization and on-demand `compact_conversation` tool | `src/domain/factory.py` |
...
| Prompt Catalog | File-based prompt management layer loading externalized Markdown templates (`prompts/*.md`) with safe variable interpolation | `src/domain/prompts.py` |
