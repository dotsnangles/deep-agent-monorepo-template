...
1. **Dual-Environment Presets (`DEEP_AGENT_MODE`)**:
   - Introduce `DEEP_AGENT_MODE=local_slm | production_cloud` in `src/core/config.py` (defaulting to `local_slm` when `LLM_PROVIDER=ollama`).
...
3. **Environment-Aware Deep Agent Factory (`src/graphs/chat/factory.py`)**:
   ...
   - In `production_cloud` mode: Register provider profiles, multi-tier `CompositeBackend` (`StoreBackend` for cross-thread memory + `DockerSandboxBackend`), specialized declarative subagents, fault-tolerant retry/fallback middlewares, and prompt caching.
...
- **Positive**:
  ...
  - **Seamless Cloud Scale-Out**: Flipping to `production_cloud` unlocks full subagent swarms, cross-session long-term memory, and LLM-as-a-judge evaluation without changing application code.
