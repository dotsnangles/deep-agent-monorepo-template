...
Some runtimes cannot pause a run mid-tool the way LangGraph's
`interrupt()` does, so this demo uses `useFrontendTool` with a
Promise-based handler instead. The agent calls `schedule_meeting` like
