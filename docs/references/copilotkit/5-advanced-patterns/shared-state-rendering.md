...
`useAgent` works in any component under ``. It doesn't have to be
...
function Canvas() {
  ...
  const state = (agent.state ?? {}) as Partial;
...
The agent lives on the `` provider, so the chat surface and your
...
> [!NOTE]
> `` and `` both call `useAgent()` for the same `agentId`,
